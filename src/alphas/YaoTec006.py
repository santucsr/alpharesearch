# Technical 006: directional index
# DX = |PDI - NDI| / (PDI + NDI)
# signal = EMA of DX
...
class YaoTec006():
    ...
    def __compute_dx(self, x):
        '''function to compute directional index'''
        deltaH = x.adj_high - x.adj_high.shift(1)
        deltaL = x.adj_low.shift(1) - x.adj_low
        PDM = deltaH * (deltaH > deltaL) * (deltaH > 0)
        NDM = deltaL * (deltaL > deltaH) * (deltaL > 0)
        ...
        PDI = PDI.ewm(span=self.__window,
                       min_periods=self.__window, adjust=False).mean().shift(1)
        
        NDI = NDM / ATR
        NDI = NDI.replace([np.inf, -np.inf], np.nan)
        NDI = NDI.ewm(span=self.__window,
                      min_periods=self.__window, adjust=False).mean().shift(1)
        
        DX = np.abs(NDI - PDI) / (NDI + PDI)
        DX = DX.replace([np.inf, -np.inf], np.nan)
        
        # return DX.ewm(span=self.__window,
        #               min_periods=self.__window, adjust=False).mean()
        return -DX
    ...
    def compute(self):
        ...
        alpha = stocks.groupby('code').apply(
            self.__compute_dx).reset_index().rename(columns={0: 'alpha'})
