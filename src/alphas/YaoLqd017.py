# Liquidity 017: Bollinger z score on 5min open turnover - 5min close turnover, -(Turnover - MA) / sigma
#
...
class YaoLqd017():
    ...
    def compute(self):
        ...
        stocks = stocks.reset_index().merge(intray_trv.reset_index(),
                                            on=['time', 'code'], how='left').set_index('time')
        # adj close
        stocks['adj_close'] = stocks['close'] * stocks['cum_adjf']
        
        stocks['trv_diff'] = stocks.am_open_5min_trv - stocks.pm_close_5min_trv

        stocks['alpha'] = stocks.groupby(
            'code').trv_diff.apply(self.__compute_bb)
