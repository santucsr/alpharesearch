# Technical 004: positive directional indicator
# +DM = H_t - H{t-1} if H_t - H_{t-1} > L_{t-1} - L_t and H_t > H_{t-1}
# PDI = EMA of (+DM/ATR)
# signal = PDI
...
class YaoTec004():
    ...
    def compute(self):
        ...
        alpha = stocks.groupby('code').apply(
            self.__compute_pdi).reset_index().rename(columns={0: 'alpha'})
