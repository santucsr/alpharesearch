# Technical 003: average true range
# TR= max(H_t, C_{t-1}) - min(L_t, C_{t-1})
# signal = moving average of (TR / C_{t-1}) ?
...
class YaoTec003():
    ...
    def __compute_atr(self, x):
        '''function to compute average true range'''
        TR = np.maximum(x.adj_high, x.adj_close.shift(1)) - np.minimum(x.adj_low, x.adj_close.shift(1))
        TR = TR / x.adj_close.shift(1)
        return -TR.ewm(span=self.__window,
    ...
    def compute(self):
        ...
        alpha = stocks.groupby('code').apply(
            self.__compute_atr).reset_index().rename(columns={0: 'alpha'})
