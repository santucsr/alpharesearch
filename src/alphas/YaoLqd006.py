# Liquidity 006: consistent turnover
# mean(consisten_turnover / turnover, N); consistent_turnover = sum(if(|close - open| <= a|high - low|, min_turnover, else 0))
...
class YaoLqd006():
    ...
    def __compute_consistency(self, x):
        ...
        return (x[f'consistent_turnover_{self.__a}'] / x['total_turnover']).rolling(self.__window, min_periods=self.__window, closed='left').mean()
