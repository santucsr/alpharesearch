# Moneyflow 015: small net flow (inflow - outflow) / market cap, the compute N day mean
...
class YaoMfl015():
    ...
    def compute(self):
        ...
        mflows = dataloader.loading(tab_name="MoneyFlow2", start=self.__start, end=self.__end, fields=[
                                    'code', 'small_outflow_trv', 'small_inflow_trv'])
        ...
        stocks['netflow'] = stocks.small_inflow_trv - stocks.small_outflow_trv
        stocks['netflow'] = stocks.netflow / stocks['neg_mkt_val']
        stocks['alpha'] = stocks.groupby('code').netflow.apply(lambda x: x.rolling(
            self.__window, min_periods=self.__window, closed='left').mean())
