# Moneyflow 003: mega net inflow (inflow - outflow) turnover ratio, inflow is defined as close > open
# - mean(mega (inflow  - outflow) turnover / total turnover, N);
...
class YaoMfl003():
    ...
    def __compute_liquidity(self, x):
        '''function to compute buy driven liquidity'''
        return -((x['mega_inflow_trv'] - x['mega_outflow_trv']) / x['total_turnover']).rolling(self.__window, min_periods=self.__window, closed='left').mean()
    ...
    def compute(self):
        ...
        mflows = dataloader.loading(tab_name="MoneyFlow", start=self.__start, end=self.__end, fields=['code', 'mega_outflow_trv', 'mega_inflow_trv'])
