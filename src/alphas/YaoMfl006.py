# Moneyflow 006: mid inflow turnover ratio, we split one minute turnover into inflow part and outflow part, based on price trajectory
# mean(mid inflow turnover / total turnover, N);
...
class YaoMfl006():
    ...
    def __compute_liquidity(self, x):
        ...
        return (x['mid_inflow_trv'] / x['total_turnover']).rolling(self.__window, min_periods=self.__window, closed='left').mean()
