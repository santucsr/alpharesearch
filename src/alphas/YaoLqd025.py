# Reversion 025: turnover adjusted abosulte return, illiquidity ratio
# |R| / turnover
# Another implementation: here we first compute multiday returns, then calculate vol adjusted returns
...
class YaoLqd025():
    ...
    def __compute_var(self, x):
        '''function to compute turnover adjusted return'''
        ret = np.abs(np.log(x.adj_close.shift(1)) - np.log(x.adj_close.shift(2)))
        return ret.rolling(self.__window).sum() / x.total_turnover.shift(1).rolling(self.__window).sum()
    ...
    def compute(self):
        ...
        cum_adjf = dataloader.loading('Cum Adj Factor', start=self.__start, end=self.__end, fields=['code', 'cum_adjf'])
        logger().debug("Loading Cumulative Adjust Factor...Done!")

        logger().debug("Loading Universe...")
        population = dataloader.loading("Universe", start=self.__start, end=self.__end, fields=self.__universe)
        ...
        alpha = stocks.groupby('code').apply(self.__compute_var).reset_index().rename(columns={0: 'alpha'})
