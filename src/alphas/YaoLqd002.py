# Liquidity 002: buyer driven turnover
# mean(buy turnover / total turnover, N); 
...
class YaoLqd002():
    ...
    def __compute_liquidity(self, x):
        ...
        return -(x['buy_turnover'] / x['total_turnover']).rolling(self.__window, min_periods=self.__window, closed='left').mean()
    ...
    def compute(self):
        ...
        logger().debug("Loading Turnover Buy Sell...")
        consistent_volumes = dataloader.loading(
            tab_name="Turnover Buy Sell", start=self.__start, end=self.__end, fields=['code', 'buy_turnover', 'sell_turnover'])
        logger().debug("Loading Turnover Buy Sell...Done!")
