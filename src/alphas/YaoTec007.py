# Technical 007: chaikin money flow
# CLV = ((C-L) - (H-C)) / (H-L)
# MFV = CLV * V
# CMF = sum(MFV) / sum(V)
...
class YaoTec007():
    ...
    def __compute_cmf(self, x):
        '''function to compute chaikin money flow index'''
        clv = ((x.adj_close - x.adj_low) - (x.adj_high - x.adj_close)) / (x.adj_high - x.adj_low)
        clv = clv.replace([np.inf, -np.inf], np.nan)
        mfv = clv * x.total_volume
        cmf = mfv.rolling(self.__window, min_periods=self.__window,closed='left').sum() / x.total_volume.rolling(self.__window, min_periods=self.__window,closed='left').sum()
        return -cmf
    ...
    def compute(self):
        ...
        alpha = stocks.groupby('code').apply(
            self.__compute_cmf).reset_index().rename(columns={0: 'alpha'})
