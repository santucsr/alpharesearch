...
class YaoVol014():
    ...
    def __compute_kurtosis(self, x):
        '''function to compute rolling kurtosis of returns'''
        ret = np.log(x.shift(1)) - np.log(x.shift(2))
        return pd.Series(ret.rolling(self.__window).kurt())
    ...
    def compute(self):
        ...
        stocks['alpha'] = stocks.groupby(
            'code').adj_close.apply(self.__compute_kurtosis)
