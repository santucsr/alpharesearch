# YaoVol010: std cross over?
# std cross over, (M day std - N day std)
...
class YaoVol010():
    ...
    def __compute_std_xo(self, x):
        '''function to compute rolling standard deviation of returns'''
        ret = np.log(x.shift(1)) - np.log(x.shift(2))
        short = pd.Series(ret.rolling(self.__shortTerm).std())
        long = pd.Series(ret.rolling(self.__longTerm).std())
        return short - long
    ...
    def compute(self):
        ...
        stocks['alpha'] = -stocks.groupby(
            'code').adj_close.apply(self.__compute_std_xo)
