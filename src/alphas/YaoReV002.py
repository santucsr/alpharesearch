# Reversion 002: price MA cross over, -(M day MA - N day MA) / N day MA, M < N
...
class YaoReV002():
    ...
    def __compute_ppo(self, x):
        '''function to compute PPO'''
        short = x.rolling(self.__shortTerm,
                          min_periods=self.__shortTerm, closed='left').mean()
        long = x.rolling(self.__longTerm,
                         min_periods=self.__longTerm, closed='left').mean()
    ...
    def compute(self):
        ...
        stocks['alpha'] = stocks.groupby(
            'code').adj_close.apply(self.__compute_ppo)
