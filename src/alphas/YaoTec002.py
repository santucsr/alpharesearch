...
class YaoTec002():
    def __init__(self, args_dict):
        ...
        self.__slowPeriod = args_dict['slowPeriod']
        # self.__delay = args_dict['delay'] # Delay 0? Delay 1?
        self.__refresh = True if 'refresh' not in args_dict else args_dict['refresh']

    def __compute_so(self, x):
        '''function to compute stochastic oscillator'''
        low = x.adj_low.rolling(self.__window,
                                min_periods=self.__window, closed='left').min()
        high = x.adj_high.rolling(self.__window,
                                min_periods=self.__window, closed='left').max()
        KFast = (x.adj_close.shift(1) - low) / (high - low)
        KFast = KFast.replace([np.inf, -np.inf], np.nan)
  
        return -KFast.rolling(self.__slowPeriod).mean()
    ...
    def compute(self):
        ...
        alpha = stocks.groupby('code').apply(
            self.__compute_so).reset_index().rename(columns={0: 'alpha'})
        ...
        dates.sort()
        dates = dates[(self.__window + self.__slowPeriod - 2):]

        self.__save(stocks, dates)

    def __save(self, df, dates):
        tableName = f"{self.__name}-{self.__window}days-{self.__slowPeriod}days-{self.__universe}"
