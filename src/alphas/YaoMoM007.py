...
class YaoMoM007():
    ...
    def __compute_var(self, x):
        '''function to compute volumne adjusted return'''
        adv = x.total_volume.rolling(self.__window,
                                     min_periods=self.__window, closed='left').mean()
        ret = np.log(x.adj_close.shift(1)) - \
            np.log(x.adj_close.shift(1 + self.__window))
        # ret = np.log(x.adj_close.shift(1)) - np.log(x.adj_close.shift(2))
        # ret = ret * adv / x.total_volume.shift(1)
        return ret * adv / x.total_volume.shift(1)
        # return -ret.rolling(self.__window).sum()
