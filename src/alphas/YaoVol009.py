# Volatility 009: N day upside standard deviation., low volatility anomaly
...
class YaoVol009():
    ...
    def __compute_upside_std(self, x):
        ret = np.log(x.shift(1)) - np.log(x.shift(2))
        return pd.Series(ret.rolling(self.__window).apply(self.__upside_std, raw=True))

    # Define vectorized functions to calculate downside standard deviation
    def __upside_std(self, x):
        neg_returns = x[x > 0]
    ...
    def compute(self):
        ...
        stocks['alpha'] = -stocks.groupby(
            'code').adj_close.apply(self.__compute_upside_std)
