# Technical 012: relative strength index
# gain = C_t - C_{t-1} if C_t > C_{t-1}, loss = C_{t-1} - C_t if C_t < C_{t-1}
# meain gain = EMA gain, mean loss = EMA loss
# RSI = mean gain / (mean gain + mean loss)
...
class YaoTec012():
    ...
    def __compute_rsi(self, x):
        '''function to compute relative strength index'''
        delta = x - x.shift(1)
        gain = delta * (delta > 0)      
        loss = -delta * (delta < 0)
        mean_gain = gain.shift(1).ewm(span=self.__window,
                          min_periods=self.__window, adjust=False).mean()
        mean_loss = loss.shift(1).ewm(span=self.__window,
                          min_periods=self.__window, adjust=False).mean()
        rsi = mean_gain / (mean_gain + mean_loss)
    ...
    def compute(self):
        ...
        stocks['alpha'] = -stocks.groupby(
            'code').adj_close.apply(self.__compute_rsi)
