# Liquidity 015: liquidity = close_5min - open_5min
# signal = mean(liquidity, N)
#
...
class YaoLqd015():
    ...
    def __compute_liquidity(self, x):
        return -(x['am_open_5min_trv'] - x['pm_open_5min_trv']).rolling(self.__window, min_periods=self.__window, closed='left').mean()
    ...
    def compute(self):
        ...
        intray_trv = dataloader.loading(
            tab_name="Intraday Turnover", start=self.__start, end=self.__end, fields=['code', 'am_open_trv', 'pm_close_trv', 'am_close_trv', 'pm_open_trv',
        ...
        stocks = stocks.reset_index().merge(intray_trv.reset_index(), on=['time', 'code'], how='left').set_index('time')
        # adj close
