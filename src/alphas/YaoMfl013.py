# Moneyflow 013: large net flow -(inflow - outflow) / market cap, the compute N day mean
...
class YaoMfl013():
    ...
    def compute(self):
        ...
        logger().debug("Loading money flows...")
        mflows = dataloader.loading(tab_name="MoneyFlow2", start=self.__start, end=self.__end, fields=['code', 'large_outflow_trv', 'large_inflow_trv'])
        logger().debug("Loading money flows...Done!")
        ...
        # merge PV data with buy sell turnovers
        stocks = stocks.reset_index().merge(mflows.reset_index(),
                                            on=['time', 'code'], how='left').set_index('time')
        ...
        stocks['netflow'] = stocks.large_inflow_trv - stocks.large_outflow_trv
        stocks['netflow'] = stocks.netflow /  stocks['neg_mkt_val']
        stocks['alpha'] = -stocks.groupby('code').netflow.apply(lambda x: x.rolling(self.__window, min_periods=self.__window, closed='left').mean())
