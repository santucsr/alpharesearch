# Liquidity 009: intraday turnover std
# liquidity = intraday turnover / total turnover
...
class YaoLqd009():
    ...
    def __compute_liquidity(self, x):
        ...
        return -(x['turnover_std'] / x['total_turnover']).rolling(self.__window, min_periods=self.__window, closed='left').mean()
    ...
    def compute(self):
        ...
        stocks = dataloader.loading(
            tab_name="PV Basics", start=self.__start, end=self.__end, fields=['code', 'close', 'total_turnover', 'turnover_std'])
