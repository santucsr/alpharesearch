# Moneyflow 005: large inflow turnover ratio, we split one minute turnover into inflow part and outflow part, based on price trajectory
# - mean(large inflow turnover / total turnover, N);
...
class YaoMfl005():
    ...
    def __compute_liquidity(self, x):
        ...
        return -(x['large_inflow_trv'] / x['total_turnover']).rolling(self.__window, min_periods=self.__window, closed='left').mean()
