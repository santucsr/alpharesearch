# Moneyflow 002: mega outflow turnover ratio, outflow is defined as close < open
# - mean(mega outflow turnover / total turnover, N);
...
class YaoMfl002():
    ...
    def __compute_liquidity(self, x):
        ...
        return -(x['mega_outflow_trv'] / x['total_turnover']).rolling(self.__window, min_periods=self.__window, closed='left').mean()
    ...
    def compute(self):
        ...
        mflows = dataloader.loading(tab_name="MoneyFlow", start=self.__start, end=self.__end, fields=['code', 'mega_outflow_trv'])
