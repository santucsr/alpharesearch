# Liquidity 021: N day mean on illiquidity1_hl_mean adjusted by market cap
#
...
class YaoLqd021():
    ...
    def compute(self):
        ...
        logger().debug("Loading Intraday Turnover...")
        illiquidity = dataloader.loading(
            tab_name="Illiquidity", start=self.__start, end=self.__end, fields=['code', 'illiquidity1_hl_mean', 'illiquidity1_hl_std', 'illiquidity1_hl_range', 'illiquidity1_ho_mean',
                                                                                'illiquidity1_ho_std', 'illiquidity1_ho_range', 'illiquidity1_co_mean', 'illiquidity1_co_std', 'illiquidity1_co_range'])
        logger().debug("Loading Intraday Turnover...Done!")
        ...
        # merge PV data with Intraday Turnover
        stocks = stocks.reset_index().merge(illiquidity.reset_index(),
                                            on=['time', 'code'], how='left').set_index('time')
        ...
        stocks['illiquidity1_hl_mean_adj'] = stocks['illiquidity1_hl_mean'] * stocks['neg_mkt_val']

        stocks['alpha'] = stocks.groupby('code').illiquidity1_hl_mean_adj.apply(lambda x: x.rolling(self.__window, min_periods=self.__window, closed='left').mean())
