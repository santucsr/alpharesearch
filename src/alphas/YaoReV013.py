...
class YaoReV013():
    ...
    def __compute_consistency(self, x):
        '''function to compute buying consistency'''
        # should flip sign here?
        return -(x[f'consistent_buy_{self.__a}'] / x['total_volume']).rolling(self.__window, min_periods=self.__window, closed='left').mean()
    ...
    def compute(self):
        ...
        logger().debug("Loading consistent buy...")
        consistent_volumes = dataloader.loading(
            tab_name="Consistent Buy Sell", start=self.__start, end=self.__end, fields=['code', 'consistent_buy_0.1', 'consistent_buy_0.2', 'consistent_buy_0.3', 'consistent_buy_0.4', 'consistent_buy_0.5', 'consistent_buy_0.6', 'consistent_buy_0.7', 'consistent_buy_0.8', 'consistent_buy_0.9'])
        logger().debug("Loading consistent buy...Done!")
