# Technical 008: money flow index
# positive money flow: +MF = TP * V if TP > TP_{t-1} otherwise 0
# negative money flow: -MF = TP * V if TP < TP_{t-1} otherwise 0
# MFI = sum(+MF) / (sum(+MF) + sum(-MF))
...
class YaoTec008():
    ...
    def __compute_mfi(self, x):
        '''function to compute money flow index'''
        TR = np.maximum(x.adj_high, x.adj_close.shift(1)) - np.minimum(x.adj_low, x.adj_close.shift(1))
        deltaTR = TR - TR.shift(1)
        pMF = TR * x.total_volume * (deltaTR > 0)
        nMF = TR * x.total_volume * (deltaTR < 0)
        mfi = pMF.rolling(self.__window, min_periods=self.__window, closed='left').sum(
        ) / (pMF.rolling(self.__window, min_periods=self.__window, closed='left').sum() + nMF.rolling(self.__window, min_periods=self.__window, closed='left').sum())
    ...
    def compute(self):
        ...
        stocks = dataloader.loading(
            tab_name="PV Basics", start=self.__start, end=self.__end, fields=['code', 'close', 'high', 'low', 'total_volume'])
        ...
        alpha = stocks.groupby('code').apply(
            self.__compute_mfi).reset_index().rename(columns={0: 'alpha'})
