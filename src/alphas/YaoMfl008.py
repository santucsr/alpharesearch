# Moneyflow 008: medium outflow turnover ratio, we split one minute turnover into inflow part and outflow part, based on price trajectory
# mean(medium outflow turnover / total turnover, N);
...
class YaoMfl008():
    ...
    def __compute_liquidity(self, x):
        ...
        return (x['mid_outflow_trv'] / x['total_turnover']).rolling(self.__window, min_periods=self.__window, closed='left').mean()
