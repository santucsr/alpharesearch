# Technical 011: divergence index
# M-day momentum * N-day momentum / N-day standand deviation 
...
class YaoTec011():
    def __init__(self, args_dict):
        self.__name = 'YaoTec011'
        self.__universe = args_dict['universe']
        self.__start = args_dict['start_date']
        self.__end = args_dict['end_date']
        self.__shortTerm = args_dict['shortTerm']
        self.__longTerm = args_dict['longTerm']
        # self.__delay = args_dict['delay'] # Delay 0? Delay 1?
        self.__refresh = True if 'refresh' not in args_dict else args_dict['refresh']
        
    def __compute_di(self, x):
        '''function to compute divergence index'''
        short = np.log(x.shift(1)) - np.log(x.shift(1 + self.__shortTerm))
        long = np.log(x.shift(1)) - np.log(x.shift(1 + self.__longTerm))
        ret = np.log(x.shift(1)) - np.log(x.shift(2))
        DI = short * long / ret.rolling(self.__longTerm).std()
        DI = DI.replace([np.inf, -np.inf], np.nan)
        return DI
    ...
    def compute(self):
        ...
        stocks['alpha'] = -stocks.groupby(
            'code').adj_close.apply(self.__compute_di)
        ...
        dates.sort()
        dates = dates[(self.__longTerm):]

        self.__save(stocks, dates)

    def __save(self, df, dates):
        tableName = f"{self.__name}-{self.__shortTerm}days-{self.__longTerm}days-{self.__universe}"
