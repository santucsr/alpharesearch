# Volatility 005: residual return skew
# r* = r - beta*rm - const
# this is a one factor model
# residual skew has momentum instead of reversion
...
class YaoVol005():
    ...
    def __compute_skew(self, x):
        '''function to compute rolling skew of returns'''
        # ret = np.log(x.shift(1)) - np.log(x.shift(2))
        return pd.Series(x.rolling(self.__window).skew())

    def __compute_rolling_capm(self, x):
        endog = x.ret_1d
        exog = sm.add_constant(x['mkt_ret_1d'])
        rols = RollingOLS(endog, exog, window=60, min_nobs=self.__window, expanding=True)
        try:
            rres = rols.fit()
    ...
    def compute(self):
        ...
        stocks['alpha'] = stocks.groupby(
            'code').alpha.apply(self.__compute_skew)
